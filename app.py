...
openai_api_chat  = "https://aiproxy.sanand.workers.dev/openai/v1/chat/completions" # for testing
...
function_definitions_llm = [
    ...
    {
        "name": "B3",
        "description": "Fetch data from an API and save it to the specified path.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
    ...
    {
        "name": "B4",
        "description": "Clone a git repository and make a commit with a given commit message.",
        "parameters": {
            "type": "object",
            "properties": {
            "repo_url": {
    ...
    },
    {
     ...
     {
        "name": "B8",
        "description": "Transcribe audio from an MP3 file and save the transcription.",
        "parameters": {
            "type": "object",
            "properties": {
                "audio_path": {"type": "string", "pattern": r".*/(.*\.mp3)", "description": "Path to the MP3 audio file to transcribe."},
                "output_filename": {"type": "string", "pattern": r".*/(.*\.txt)", "description": "Path to save the transcription text."}
            },
    ...
    },
    {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "csv_path": {"type": "string", "pattern": r".*/(.*\.csv)", "description": "Path to the CSV file."},
...
def get_completions(prompt: str):
    ...
    chosen_function = response.json()["choices"][0]["message"]["tool_calls"][0]["function"]
    print("LLM chose:", chosen_function)
...
async def run_task(task: str):
    try:
        ...
        if "B12"== task_code:
            res = B12(**json.loads(arguments))
        if "B3" == task_code:
            res = B3(**json.loads(arguments))
        if "B4" == task_code:
            res = B4(**json.loads(arguments))
        if "B5" == task_code:
            res = B5(**json.loads(arguments))
        if "B6" == task_code:
            res = B6(**json.loads(arguments))
        if "B7" == task_code:
            res = B7(**json.loads(arguments))
        if "B8" == task_code:
            res = B8(**json.loads(arguments))
        if "B9" == task_code:
            res = B9(**json.loads(arguments))
        if "B10" == task_code:
            res = B10(**json.loads(arguments))
        
        if(res):
            return {"message": f"{task_code} Task '{task}' executed successfully", "response": res}
        else:
            return {"message": f"{task_code} Task '{task}' executed"}
